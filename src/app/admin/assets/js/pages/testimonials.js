const PageTestimonials = (function () {
    ...
    let main = {
        class: {
            TESTIMONIALS: ".e_testimonials"
        },
        check(){
            let self = this;
            if(Main.variable.USER_TYPE !== TableTypes.user.ADMIN && !Variable.isset(()=> Main.variable.USER_PERMISSION[TableTypes.permission.TESTIMONIAL_EDIT])){
                $(`${self.class.TESTIMONIALS} a[function="add_new"]`).remove();
            }
        },
        init(){
            let self = this;
            ...
            function events(){
                $(`${self.class.TESTIMONIALS} a[function]`).on("click", function () {
    ...
    let list = {
        ...
        get(){
            ...
            if(self.variable.DATA.length === 0){
                Main.service(ServicePage.TESTIMONIAL, "GET",
                    {type: "all"},
                    function (data) {
                    ...
                    switch (_function){
                        ...
                        case "delete":
                            HelperSweetAlert.question(
                                "Takdir Silme",
                                `<b>'${item.title}</b> başlıklı takdiri silmek istediğinizden emin misiniz?`,
                                lang.yes,
                                lang.no,
                                function (result) {
                                    if(result.value){
                                        Main.service(ServicePage.TESTIMONIAL, "DELETE",
                                            {id: id},
                                            function (data) {
                                                element_tr.remove();
                                                Variable.del_index_of(self.variable.DATA, item._index);
                                                self.get();
                                                HelperToastify.success(`<b>'${item.title}</b> başlıklı takdir silindi`);
                                            }, true, true
                                        );
                                    }
                                }
            ...
            function events(){
                $(`${self.id.MODAL} form`).submit(function (e) {
                   ...
                   form_data = Object.assign(
                    ...
                    Main.service(ServicePage.TESTIMONIAL, ((self.variable.MODE === self.variable.mode.ADD) ? "POST" : "PUT"),
                        form_data,
                        function (data) {
                            HelperToastify.success(`<b>'${form_data.title}'</b> isimli takdir başarı ile ${(self.variable.MODE === self.variable.mode.ADD) ? "oluşturuldu" : "güncellendi"}`);
    ...
    return PageTestimonials;
})();

$(function () {
    (new PageTestimonials());
})