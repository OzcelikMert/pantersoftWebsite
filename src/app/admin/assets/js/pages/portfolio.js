let PagePortfolio = (function () {
    function PagePortfolio(){
        category.init()
        portfolio.init();
        //Modals
        modal_category.init();
        modal_portfolio.init();
        tags.init();
    }
    let portfolio = {
        variable: {
        ...
        class: {
            PORTFOLIO_AREA: ".e_portfolio_area",
            TRASH_PORTFOLIO: ".e_trash_portfolio",
            PORTFOLIO: ".e_portfolio",
        },
        get(){
            let self = this;
            Main.service(ServicePage.PORTFOLIO,"GET",{type: "portfolio"},(data)=>{if (data.status) self.variable.DATA = data.result; }, false);
            function create_element(type="normal"){
                ...
                if (self.variable.DATA.length > 0) {
                    self.variable.DATA.forEach((item)=>{
                        ...
                        switch (type) {
                            case "normal":
                                btn =`<button class="${btn_class} bg-danger" function="trash"> <i class="bi bi-trash"></i>  </button>
                                      <button class="${btn_class} bg-warning me-2" function="edit">   <i class="bi bi-pencil"></i> </button>
                        ...
                        table_columns.push({
                            tr: {"item-id": `${item.id}`,class:"test test2"},
                            td: [
                                ...
                                `<p class="mb-0"><span class="badge ${Main.status_bg(item.status)}">${item.status_name}</span></p>`,
                                `<span>${btn}</span>`
                            ]
                        })
                    })
               ...
               return new Table({class:"e_portfolio table table-hover"})
                   .head(["İsim", "Tarih", "Kategori", "Etiket", "Durum", {html:"İşlemler", attr:{class:"text-center"}}])
                   .body(table_columns)
            }
            $(self.class.PORTFOLIO_AREA).html(create_element("normal"));
            $(self.class.TRASH_PORTFOLIO).html(create_element("trash"));

            Main.DataTable($(self.class.PORTFOLIO)[0]); //NORMAL DT
            Main.DataTable($(self.class.PORTFOLIO)[1]); //TRASH DT
            tags.get();
        },
        init(){
            let self = this;
            function events(){
                $(modal_portfolio.class.OPEN_MODAL).click(()=> {
                    modal_portfolio.clear();
                    $(`${modal_portfolio.id.MODAL} ${modal_portfolio.class.SUBMIT_BTN}`).html("Ekle");
                })
                $(document).on("click",`${self.class.PORTFOLIO} [function],${self.class.TRASH_PORTFOLIO} [function]`,function () {
                    let element = $(this),
                    ...
                    switch (type) {
                        case "edit":
                            console.log("edit")
                            modal_portfolio.clear()
                            Main.service(ServicePage.PORTFOLIO,"GET",{type: "portfolio",id: id},(data)=>{
                                if (data.result.length > 0) {
                                    ...
                                    $(`${modal_portfolio.id.MODAL} form`).autofill(data.result[0]);
                                    $(`${modal_portfolio.class.CONTENT} ${modal_portfolio.class.EDITOR}`).html(Variable.decode_html($(`${modal_portfolio.class.CONTENT} [name=content]`).val()))

                                    let image = data.result[0].image;
                                    if(image !== "") {
                                        let img = $(`${modal_portfolio.class.IMAGE} img`);
                                        img.attr("src",`../image/${image}`);
                                        img.show();
                                    }
                                    modal_portfolio.tags("text",data.result[0].tag);
                                    $(`${modal_portfolio.id.MODAL} ${modal_portfolio.class.SUBMIT_BTN}`).html("Güncelle");
                                    $(`${modal_portfolio.id.MODAL}`).modal("show");
                                }
                            }, true,true);
                            break;
                        case "trash":
                            console.log("trash")
                            HelperSweetAlert.question("Portföy Silinsinmi ?", `<b>${name}</b> Adlı Portföy'u Silmek İstediğine Eminmisiniz?`, lang.yes, lang.no,
                                function (result) {
                                    if (result.value) {
                                        Main.service(ServicePage.PORTFOLIO,"PUT",{type: "trash", id: id},(data)=>{
                                            if (data.status) {
                                                HelperToastify.success(`<b>${name}</b> Adlı Portföy Silindi.`);
                                                closest.fadeOut();
                                                self.get()
                                            }
                            ...
                            break;
                        case "restore":
                            Main.service(ServicePage.PORTFOLIO,"PUT",{type:"restore",id:id},(result)=>{
                                if (result.status) {
                                    HelperToastify.success("Portföy Geri Yüklendi.")
                                    self.get();
                                }
                            })
                            break;
                        case "delete":
                            console.log("delete")
                            HelperSweetAlert.question(`Portföy Kalıcı Olarak Silinsinmi ?`, `<b>${name}</b> Adlı Portföy Kalıcı Olarak Silinecek Eminmisiniz ?`, lang.yes, lang.no,
                                function (result) {
                                    if (result.value) {
                                        Main.service(ServicePage.PORTFOLIO,"DELETE",{type: "delete", id: id},(data)=>{
                                            if (data.status) {
                                                HelperToastify.success(`<b>${name}</b> Adlı Portföy Kalıcı Olarak Silindi.`)
                                                closest.fadeOut();
                                                self.get()
                                            }
                                        }, false);
    ...
    let tags = {
        ...
        get(){
            ...
            portfolio.variable.DATA.forEach((item)=>{
            ...
            $(self.class.SELECT).select2({
                ...
                dropdownParent: $(modal_portfolio.id.MODAL),
                width: '100%'
            })
        },
        init(){
            ...
            function events(){
                $(self.class.SELECT_RENDERED).click(()=>{
                    $(self.class.SELECT_DROPDOWN).append(`<button class="btn btn-success" id="portfolio_tag_add_btn" style="position:absolute;top:7px;right:8px;padding:2px;font-size:15px;">Oluştur</button>`);
    ...
    let category = {
        ...
        get(type="all"){
            let self = this;
            Main.service(ServicePage.CATEGORY,"GET",{type: type,category_type: TableTypes.category.PORTFOLIO}, (data)=> {
                if (data.status) {
                    ...
                    self.variable.DATA.forEach((item)=>{
                        ...
                        table_columns.push({
                            ...
                            td: [
                                ...
                                main_category,
                                item.date,
                                `<a href="portfolio/${item.url}">${item.url}</a>`,
        ...
        get_linked(portfolio_id){
            let self = this;
            let category = [];
            let category_id = [];
            let linked = Variable.array_find_multi(self.variable.DATA_LINKED,"item_id",portfolio_id)
    ...
    let modal_portfolio = {
        id: {
          MODAL: "#modal_portfolio",
          TAG_BTN: "#portfolio_tag_add_btn",
        },
        class: {
            CONTENT : ".e_content",
            EDITOR: ".note-editable",
            CATEGORY: ".e_categories",
            IMAGE: ".e_portfolio_image",
            OPEN_MODAL: ".e_open_modal_portfolio",
            TAGS: ".e_tags",
        ...
        set(data,method="POST"){
            data.type = "portfolio";
            Main.service(ServicePage.PORTFOLIO, method, data,function (data) {
                if (data.status){
        ...
        get() {
            ...
            function create_element_category(){
                ...
                return `<li class="list-group-item"> <label class="bold">Kategori Seçimi</label></li> ${element}`;
            }
    ...
    let modal_category = {
        ...
        set(data,method="POST"){
            let self = this;
            data.type = "category";
            data.category_type = TableTypes.category.PORTFOLIO;
