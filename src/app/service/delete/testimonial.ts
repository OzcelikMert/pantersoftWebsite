...
import V, {ClearTypes, DateMask, Permisson} from "../../../pantersoft/server/operation/variable";
...
class Testimonial {
    ...
    private set() {
        ...
        let values = [
            {key: tbl.testimonial.STATUS, value: TableTypes.status.DELETED, type: var_types.NUMBER},
        ];

        db.update(tbl.testimonial.TABLE_NAME).columns(values).where.equals([{key: tbl.testimonial.ID, value: self.data.id, type: var_types.NUMBER}]).run();
    }
    ...
    private check_values(){
        ...
        if(self.result.error_code == ErrorCode.SUCCESS){
            self.result.error_code = Permisson.control(self.session, TableTypes.user.ADMIN, TableTypes.permission.TESTIMONIAL_EDIT)
