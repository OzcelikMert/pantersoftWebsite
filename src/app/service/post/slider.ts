...
class Slider {
    ...
    private set() {
        ...
        let values = [
            {key: tbl.slider.TITLE(self.session.lang),   value: self.data.title},
            {key: tbl.slider.CONTENT(self.session.lang), value: self.data.content},
            {key: tbl.slider.DATE,                       value: V.date_format(new Date(), DateMask.ALL)},
            {key: tbl.slider.STATUS,                     value: (self.data.status === 1) ? 1 : 2, type: var_types.NUMBER},
            {key: tbl.slider.RANK,                       value: self.data.rank, type: var_types.NUMBER},
            {key: tbl.slider.IMAGE,                      value: self.data.image}
        ];

        db.insert(tbl.slider.TABLE_NAME).values(values).run();
    }
