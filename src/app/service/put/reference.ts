...
class Reference{
    ...
    private put() : Array<any> | any {
        this.result.result = this.db.update(tbl.reference.TABLE_NAME).columns([
        ...
        ]).where.equals([{key: tbl.reference.ID,value:this.data.id, type: var_types.NUMBER}]).run();
    }

    private check_values(){
        if (!
            (V.isset(()=> this.data.id))   &&
