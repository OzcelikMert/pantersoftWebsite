...
class Subscriber {
    ...
    private set() {
        ...
        db.update(tbl.subscribers.TABLE_NAME)
            .columns([
                {
                    key: tbl.subscribers.STATUS,
                    value: db.case.equals([
                        {column: tbl.subscribers.STATUS, value: TableTypes.status.PASSIVE, result: TableTypes.status.ACTIVE}
                    ]).else(TableTypes.status.PASSIVE),
                    type: var_types.SQL
                }
            ])
            .where.equals([{key: tbl.subscribers.ID, value: this.data.id, type: var_types.NUMBER}]).run();
    }
